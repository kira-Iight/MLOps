...
def main() -> None:
    ...
    if failed:
        # Печатаем каждое нарушение на своей строке — так его видно
        # и в терминале, и в логе check.sh (grep -c считает строки).
        import sys as _sys
        for v in failed:
            print(f"diversity: {v}", file=_sys.stderr)
        raise DiversityError(f"diversity: нарушено порогов {len(failed)}")
